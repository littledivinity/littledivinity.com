...
function normalizeBackendSiteUrl(rawUrl?: string | null): string {
  const fallback = "https://ecombeckend.saaszo.in";
  if (!rawUrl || !rawUrl.trim()) {
    return fallback;
  }
  let url = rawUrl.trim().replace(/\/+$/, "");
...
const BACKEND_SITE_URL = normalizeBackendSiteUrl(process.env.NEXT_PUBLIC_BACKEND_SITE_URL || process.env.BACKEND_SITE_URL);
